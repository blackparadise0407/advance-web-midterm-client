...
export const loadBoardByID = (id) => async (dispatch, getState) => {
  ...
  try {
    ...
    dispatch({
      type: BOARD_LOADED, payload: {
        message,
        data,
      }
    })
  } catch (error) {
    dispatch({ type: BOARD_ERROR, err: error.response.message.data || error })
  }
}

export const addAction = ({ id, action: { field, name } }) => async (dispatch, getState) => {
  dispatch({ type: BOARD_LOADING })
  const config = tokenConfig(getState)
...
export const removeAction = ({ id, action: { field, name } }) => async (dispatch, getState) => {
  dispatch({ type: BOARD_LOADING })
  const config = tokenConfig(getState)
  const body = JSON.stringify({ field, name })
  try {
